...
🔴 SADECE düşük puanlardan seçersen HATALI olur!
🔴 SADECE yüksek puanlardan seçersen HATALI olur!

Örnek doğru liste:
- Kontrolcülük: 88 (YÜKSEK) → "Gereğinden fazla süreç işletmemek"
- Duygu Kontrolü: 15 (DÜŞÜK) → "Duyguyu kontrol edebilmek"
- Kural Uyumu: 82 (YÜKSEK) → "Esnek olabilmek"
- Özgüven: 19 (DÜŞÜK) → "Karar almakta zorlanabilirsin"
- İlişki Yönetimi: 76 (YÜKSEK) → "İlişkiyi korumayı kabul etmek olmaktan ayırmak"
- Merak: 7 (DÜŞÜK) → "İnisiyatif almada çekingenlik"

ADIM 3 - 6-8 ARASI GELİŞİM ALANI GÖSTER:
- Minimum 3 tanesi YÜKSEK puanlardan (75-100)
- Minimum 3 tanesi DÜŞÜK puanlardan (0-25)
- TOPLAM: 6-8 arası gelişim alanı (ideal: 6-7)
- Dokümanından (Gelişim.md) AYNEN alıntı yap

🚨🚨🚨 KRİTİK KONTROL - YANIT VERMEDEN ÖNCE SAY! 🚨🚨🚨

Gelişim alanı listeni KONTROL ET:

1. YÜKSEK puanlardan (75-100) kaç tane var? → Minimum 3 OLMALI!
   💡 Boyut A: 83 (YÜKSEK) ← 1
   💡 Boyut B: 77 (YÜKSEK) ← 2
   💡 Boyut C: 95 (YÜKSEK) ← 3
   ✅ 3 veya daha fazla var mı?

2. DÜŞÜK puanlardan (0-25) kaç tane var? → Minimum 3 OLMALI!
   💡 Boyut D: 12 (DÜŞÜK) ← 1
   💡 Boyut E: 23 (DÜŞÜK) ← 2
   💡 Boyut F: 8 (DÜŞÜK) ← 3
   ✅ 3 veya daha fazla var mı?

3. TOPLAM kaç gelişim alanı var? → 6-8 arası OLMALI!
   ✅ 6, 7 veya 8 tane var mı?

🚨 HATALI ÖRNEKLER:
❌ 4 tane düşük + 0 tane yüksek = YANLIŞ!
❌ 2 tane düşük + 2 tane yüksek = YANLIŞ! (Toplam 4, minimum 6 olmalı)
❌ 6 tane düşük + 0 tane yüksek = YANLIŞ! (Yüksek yok)
❌ 0 tane düşük + 6 tane yüksek = YANLIŞ! (Düşük yok)

✅ DOĞRU ÖRNEK:
3 yüksek + 3 düşük = 6 toplam ✅
4 yüksek + 3 düşük = 7 toplam ✅
3 yüksek + 4 düşük = 7 toplam ✅
...
❌ "Duygusal tepkileri yönetme" ← Başlık bile AI'nin uydurması!
...
🔍 KONTROL YÖNTEMİ:
Dokümanındaki maddeyi bul → CTRL+C (kopyala) → CTRL+V (yapıştır)
ASLA kendin YAZMA! ASLA parafraz etme! ASLA kelime değiştirme!

Her kelime dokümanla AYNI olmali! Tek kelime bile değiştirme!
...
[6-8 arası gelişim alanı - her birinin maddeleri DOKÜMANIN TAM KOPYASI]
[Minimum 3 YÜKSEK + Minimum 3 DÜŞÜK]
...
NOT: "organizasyon", "stres" gibi Türkçeleşmiş kelimeler kullanılabilir.
...
- Katılımcı: ${state.participantName || 'Katılımcı'}`;
    ...
    // 📊 TOKEN KULLANIMI LOGU
    console.log('📊 Token kullanımı:', {
      stage: `Stage ${state.stage}`,
      participant: state.participantName || 'Unknown',
