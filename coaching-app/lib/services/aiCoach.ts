...
Sen:
✓ [Konuşmada GERÇEKTEN söylediği spesifik cümleyi kullan - uydurma!]
✓ [Doğru yakaladığı ikinci nokta - konuşmadan al]
✓ [Seçtiği eylem + aldığı tarih/taahhüt - konuşmadan al]

📊 SENİN PROFİLİN:

ÇOK YÜKSEK PUANLAR (75-100):
[Puanlara bakarak gerçekten 75+ olan boyutları listele - puan ile birlikte]

ÇOK DÜŞÜK PUANLAR (0-25):
[Puanlara bakarak gerçekten 0-25 olan boyutları listele - puan ile birlikte]

🎯 ÖNCELİKLİ GELİŞİM ALANLARI (Model Çözüm):

1. [EN KRİTİK ALAN] ([puan]) ✅ [Sen bunu seçtiysen işaretle]
• [Ne yapması gerektiğinden 2-3 madde]
→ Neden öncelik? [Diğer alanları nasıl tetiklediğini açıkla]

2. [İKİNCİ ALAN] ([puan])
• [Ne yapması gerektiğinden 2-3 madde]
→ Neden ikinci? [Birincisi oturduktan sonra bu neden kolaylaşacak]

3. [ÜÇÜNCÜ ALAN] ([puan])
• [Ne yapması gerektiğinden 2-3 madde]
→ Neden üçüncü? [Sıranın mantığını açıkla]

[Gerekirse 4. ve 5. alan da ekle]

GELİŞİM YOL HARİTAN:
1. ŞİMDİ (1-3 ay):
• [Somut odak - seçtiği eylemle bağlantılı]
• [Pratik adım]

2. SONRA (3-6 ay):
• [Bir sonraki gelişim alanı]
• [Nasıl ilerleyeceği]
