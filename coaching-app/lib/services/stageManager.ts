...
function evaluateStage3Transition(
  ...
  // Initial message - ASLA geçiş yapma
  if (lastUserMessage === '__STAGE_INIT__' || lastUserMessage === 'Basla') {
    return { shouldTransition: false };
  }

  // En az 4 mesaj olmadan geçme
  if (messageCount < 4) {
    return { shouldTransition: false };
  }

  // TEK geçiş koşulu: AI "geçelim mi?" sordu VE kullanıcı açıkça onay verdi
  const aiAskedToTransition = aiAsksAboutNextTopic(lastAIResponse, 3);
  const explicitConfirmation = /\b(evet|tamam|geçelim|gecelim|olur|haydi)\b/i.test(lastUserMessage.toLowerCase());

  if (aiAskedToTransition && explicitConfirmation) {
    return {
      shouldTransition: true,
      nextStage: 4,
      reason: 'AI asked AND user explicitly confirmed'
    };
  }

  // Kullanıcı doğrudan "geçelim" veya "geçmek istiyorum" dediyse
  const explicitTransition = /\b(geçelim|gecelim|geçmek\s+istiyorum|gelişim\s+alan.*geç)\b/i.test(lastUserMessage.toLowerCase());
  if (explicitTransition && messageCount >= 4) {
    return {
      shouldTransition: true,
      nextStage: 4,
