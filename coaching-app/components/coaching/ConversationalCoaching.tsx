...
export default function ConversationalCoaching() {
  ...
  useEffect(() => {
    // SADECE BİR KEZ ÇALIŞIR - boş dependency array ile
    // Stage geçişleri handleSendMessage üzerinden local state ile yönetilir
    if (!session || initializedStage.current !== null) return;

    // Her zaman stage 3'ten başla
    const startStage = 3 as CoachingState['stage'];
    console.log('✅ Initializing coaching at stage 3');
    // NOT: initializedStage.current'i sendInitialMessage set edecek
    ...
    const stageMarker: Message = {
      role: 'system',
      content: `--- ${STAGE_TITLES[startStage]} ---`
    };
  ...
  const sendInitialMessage = async (state: CoachingState) => {
    ...
    // Bu stage zaten initialize edildiyse çıkış yap
    if (initializedStage.current === state.stage) {
      console.log('⏭️ Stage', state.stage, 'already initialized, skipping');
      return;
    }

    initializedStage.current = state.stage;
    ...
    try {
      ...
      // KRİTİK FIX: Initial message stage'i DEĞİŞTİRMEMELİ
      // API ne döndürürse döndürsün, stage'i zorla orijinal stage'de tut
      const safeState = { ...data.state, stage: state.stage };
      setCoachingState(safeState);

      if (safeState.stage === 6) {
  ...
  const handleSendMessage = async (userMessage: string) => {
    ...
    try {
      ...
      if (data.state.stage !== coachingState.stage) {
        ...
        // Geçiş mesajını göster (override mesajı)
        setMessages([assistantMsg]);
        const stageMarker: Message = {
          role: 'system',
          content: `--- ${STAGE_TITLES[data.state.stage]} ---`
        };
        setAllMessages(prev => [...prev, assistantMsg, stageMarker]);

        // Yeni state'i set et
        setCoachingState(data.state);

        // YENİ STAGE'İN INITIAL MESSAGE'INI OTOMATİK GÖNDER
        const nextStageState = {
          ...data.state,
          conversationHistory: [],
        };
        setTimeout(() => {
          sendInitialMessage(nextStageState);
        }, 500);
        return;
      } else {
        setMessages([...updatedMessages, assistantMsg]);
