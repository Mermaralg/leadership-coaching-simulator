...
export default function ConversationalCoaching() {
  ...
  useEffect(() => {
    console.log('🔵 useEffect triggered - currentStage:', session?.currentStage);
    
    if (!session) return;
    
    if (initializedStage.current === session.currentStage) {
      console.log('⏭️ Stage already initialized, skipping');
      return;
    }

    console.log('✅ Initializing stage:', session.currentStage);
